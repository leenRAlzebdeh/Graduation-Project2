  using JUSTLockers.Classes;
    ...
    namespace JUSTLockers.Services
    {
        ...
        public interface IEmailService
        {
            ...
            Task SendStudentsNotificationAsync(List<string> recipients, EmailMessageType messageType, Dictionary<string, string>? data = null);
            ...
            Task SemesterEndNotificationAsync(List<string> recipient, string messageType, Dictionary<string, DateTime>? data = null);
        }
        ...
        public class EmailService : IEmailService
        {
            ...
            {
                await SendEmailAsync(recipients[0].ToString(), subject, body);
                return;
            }
            ...
            public async Task SemesterEndNotificationAsync(List<string> recipient, string messageType, Dictionary<string, DateTime>? data = null)
            {
                (string subject, string body) = GenerateEmailContent(messageType, data);
                Console.WriteLine(subject + body);
                await SendEmailToManyAsync(recipient, subject, body);
            }
            ...
            private (string subject, string body) GenerateEmailContent(string messageType, Dictionary<string, DateTime>? data)
            {
                string subject = "";
                string body = "";
                ...
                switch (messageType)
                {
                    case "StudentSemesterEndNotification":
                        subject = "Semester End Notification";
                        body = $"Dear Student,\n\n" +
                               "The semester is ending soon.\n" +
                               $"End Date: {data?.GetValueOrDefault("EndDate").ToString("yyyy-MM-dd") ?? "N/A"}\n" +
                               "Please prepare to return your locker.\n\n" +
                               "Best regards,\nJUSTLockers Team";
                        break;
                    case "StudentSemesterEndFinalNotification":
                        subject = "Final Semester End Notification";
                        body = $"Dear Student,\n\n" +
                               "The semester has ended.\n" +
                               $"End Date: {data?.GetValueOrDefault("EndDate").ToString("yyyy-MM-dd") ?? "N/A"}\n" +
                               "Please return your locker as soon as possible.\n\n" +
                               "Best regards,\nJUSTLockers Team";
                        break;
                    case "SupervisorsSemesterEndNotification":
                        subject = "Semester End Notification";
                        body = $"Dear Supervisor,\n\n" +
                               "The semester is ending soon.\n" +
                               $"End Date: {data?.GetValueOrDefault("EndDate").ToString("yyyy-MM-dd") ?? "N/A"}\n" +
                               "Please prepare your student for that .\n\n" +
                               "Best regards,\nJUSTLockers Team";
                        break;
